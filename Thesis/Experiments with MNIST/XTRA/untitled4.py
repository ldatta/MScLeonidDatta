...
class Net(nn.Module):
    def __init__(self):
        ...
        #self.conv5 = nn.Conv2d(15, 10, 3, 1)
...
# =============================================================================
        ...
        #print("length after maxpool",len(x))
        #print(x.shape) #64 50 4 4 
        #x = x.view(-1, 4*4*50)
        #print("length after dot view",len(x))
        #print(x.shape) #64 800
        #x=self.fc1(x)
        #x = F.relu(x)
        #x = self.fc2(x) 
        x = self.GAP(x)
        #print("gap done")
        #print("after GAP",len(x))
        #print(x.shape)
        #whatever this is , to get [10, 1, 1]
        
        x = x.view(-1, 10)
        #print("after GAP",len(x))
        #print(x.shape)
        x=F.log_softmax(x, dim=1)
        #print("final from forward",len(x))
        #print(x.shape)
        return x
