...
class Net(nn.Module):
    ...
    def forward(self, x):
        x=x.float()
        x=self.conv1(x) #25 x 26 x 26
# =============================================================================
#         def printimg(W1):
#             W1=W1.detach.numpy()
#             for i in range (3):
#                 plt.imshow(W1[1,i,:,:], interpolation='nearest')
#                 plt.show() 
# =============================================================================
        W1=x.numpy()
        for i in range(3):
        ...
        x = F.relu(x)
        x = F.max_pool2d(x,2, 2) #maxpool 1 25 x 13 x 13
        #print(x.shape,"maxpool 1")
        W1=x.numpy()
        for i in range(3):
            print("after maxpool 1")
            plt.imshow(W1[1,i,:,:], interpolation='nearest')
            plt.show()
        x = F.relu(self.conv2(x)) #22x 11 x 11
        W1=x.numpy()
        for i in range(3):
            print("after conv 2")
            plt.imshow(W1[1,i,:,:], interpolation='nearest')
        ...
        x = F.max_pool2d(x, 2, 2) #22 x 5 x 5
        #print(x.shape,"maxpool 2")
        x = F.relu(self.conv3(x)) #10 x 3 x 3
        ...
        x=x.float()
        print(x.shape,"after cor") #10 x 3 x 3
        x = self.GAP(x)
        x = x.view(-1, 10) 
...
print("red test data done above")
