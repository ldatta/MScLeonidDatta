...
#Directly load correlation
# =============================================================================
# a=np.load('cortrainred.npy')  #red training data 60000*28*28*1
# #
# b=np.load('cortestgreen.npy')   #green test data 10000*28*28*1
# 
# a2=np.load('trainlabel.npy')  #red training data labels
# 
# b2=np.load('testlabel.npy') #green testing data labela
# 
# c=np.load('cortestred.npy')  
# 
# c2=np.load('testlabel.npy')
# a=a.reshape(a.shape[0],1,28,28)
# b=b.reshape(b.shape[0],1,28,28)
# c=c.reshape(c.shape[0],1,28,28)
# #a=np.transpose(a,(0,3, 1, 2))  
# #b=np.transpose(b,(0,3, 1, 2))   
# #c=np.transpose(c,(0,3, 1, 2))
# 
...
#         cor1[i,0,:,:]=np.zeros((cor1[i,0,:,:].shape))#C.correlate(img[i,0,:,:],img[i,0,:,:], output=None, mode='constant', cval=0.0, origin=0)
#         cor1[i,1,:,:]=np.zeros((cor1[i,0,:,:].shape))#C.correlate(img[i,1,:,:],img[i,1,:,:], output=None, mode='constant', cval=0.0, origin=0)
#         cor1[i,2,:,:]=np.zeros((cor1[i,0,:,:].shape))#C.correlate(img[i,2,:,:],img[i,2,:,:], output=None, mode='constant', cval=0.0, origin=0)
#         cor1[i,3,:,:]=C.correlate(img[i,0,:,:],img[i,1,:,:], output=None, mode='constant', cval=0.0, origin=0)
#         cor1[i,4,:,:]=C.correlate(img[i,0,:,:],img[i,2,:,:], output=None, mode='constant', cval=0.0, origin=0)
#         cor1[i,5,:,:]=C.correlate(img[i,1,:,:],img[i,2,:,:], output=None, mode='constant', cval=0.0, origin=0)
#         sum1[i,:,:]=cor1[i,0,:,:]+cor1[i,1,:,:]+cor1[i,2,:,:]+cor1[i,3,:,:]+cor1[i,4,:,:]+cor1[i,5,:,:]
...
def cor(img):
    ...
    for i in range (img.shape[0]):
        cor1[i,:,:,0]=C.correlate(img[i,:,:,0],img[i,:,:,0], output=None, mode='constant', cval=0.0, origin=0)
        cor1[i,:,:,1]=C.correlate(img[i,:,:,1],img[i,:,:,1], output=None, mode='constant', cval=0.0, origin=0)
        cor1[i,:,:,2]=C.correlate(img[i,:,:,2],img[i,:,:,2], output=None, mode='constant', cval=0.0, origin=0)
        #print("done")
        #print(cor1[i,:,:,2].shape, "cor[i,:,:,2] shape")
        cor1[i,:,:,3]=np.zeros((img[i,:,:,0].shape))#C.correlate(img[i,:,:,0],img[i,:,:,1], output=None, mode='constant', cval=0.0, origin=0)
        cor1[i,:,:,4]=np.zeros((img[i,:,:,0].shape))#C.correlate(img[i,:,:,0],img[i,:,:,2], output=None, mode='constant', cval=0.0, origin=0)
        cor1[i,:,:,5]=np.zeros((img[i,:,:,0].shape))#C.correlate(img[i,:,:,2],img[i,:,:,1], output=None, mode='constant', cval=0.0, origin=0)
        sum1[i,:,:]=cor1[i,:,:,0]+cor1[i,:,:,1]+cor1[i,:,:,2]+cor1[i,:,:,3]+cor1[i,:,:,4]+cor1[i,:,:,5]
...
class Net(nn.Module):
    ...
    def forward(self, x):
        #print(x.shape,"before cor")
        x=x.float()
        #x=cor(x)
        #print(x.shape,"after cor")
        x=x.float()
        x=self.conv1(x)
        print(x.shape,"after convo1")
        a=np.zeros(([64,25,26,26]))
        print("a done")
        W1=x.detach().numpy()
        for i in range(10):
            plt.imshow(W1[0,i,:,:], interpolation='nearest')
            plt.show()
...
br=0#test(args, model, device, redtest_loader)
