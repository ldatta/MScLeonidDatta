...
b=np.load('/Users/leoniddatta/Dropbox/TU Delft/Study material/DL THESIS/Experiments with MNIST/verTestData.npy')   #VERTICAL test data 10000*28*28*1
b2=np.load('/Users/leoniddatta/Dropbox/TU Delft/Study material/DL THESIS/Experiments with MNIST/testlabel.npy') 
c=np.load('/Users/leoniddatta/Dropbox/TU Delft/Study material/DL THESIS/Experiments with MNIST/horTestData.npy')  #HORIZONTAL test data
...
def cor(img,img2):
    ...
    img4=np.reshape(img2,(img2.shape[0],img2.shape[2],img2.shape[3])) 
    print("image is")
    plt.imshow(img3[1],cmap='gray', interpolation='nearest')
    plt.show()
    print("template is")
    plt.imshow(img4[1,:,:], cmap='gray',interpolation='nearest')
    plt.show()
    cor1=np.zeros((img.shape[0],1,28,28))
    for i in range (img.shape[0]):
        cor1[i,:,:,:]=C.correlate(img[i,:,:,:],img2[i,:,:,:], output=None, mode='constant', cval=0.0, origin=0)
    print("correlation is")
    print(cor1.shape)
    cor=np.reshape(cor1,(cor1.shape[0],cor1.shape[2],cor1.shape[3])) 
    plt.imshow(cor[1],cmap='gray', interpolation='nearest')
    plt.show()
...
# =============================================================================
#         img=x.detach().numpy()
#         img=np.reshape(img,(img.shape[0],28,28))
