...
def cor(img,img2):
    ...
    #print(img2.shape,"template shape")
    #img3=np.transpose(img,(0,2, 3, 1))
    #img4=np.transpose(img2,(0,2, 3, 1))
    #print(img.shape,"image shape after trns")
    #print(img2.shape,"template shape after trans")
    #sum=np.zeros((img.shape[0],img.shape[1],img.shape[2],img.shape[3]))
    ...
    for i in range (img.shape[0]):
        ...
        #cor1[i,2,:,:]=C.correlate(img[i,2,:,:],img2[i,2,:,:], output=None, mode='constant', cval=0.0, origin=0)
            #sum[i,j,:,:]=cor1[i,0,:,:]#+cor1[i,1,:,:]+cor1[i,2,:,:] #+cor1[i,3,:,:]+cor1[i,4,:,:]+cor1[i,5,:,:]
    print("image is")
    plt.imshow(img[1,1,:,:], cmap='gray', interpolation='nearest')
    plt.show()
    print("template is")
    plt.imshow(img2[1,1,:,:],cmap='gray',  interpolation='nearest')
    plt.show()
    print("correlation is")
    plt.imshow(cor1[1,1,:,:], cmap='gray', interpolation='nearest')
    plt.show()
    #sum=np.reshape(sum,(img.shape[0],1,img.shape[2],img.shape[3])) 
