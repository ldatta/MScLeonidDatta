...
c=np.load('/Users/leoniddatta/Dropbox/TU Delft/Study material/DL THESIS/Experiments with MNIST/test.npy')  
...
def cor(img):
    ...
    for i in range (img.shape[0]):
        ...
        for j in range (img.shape[1]):
            cor1[i,j,:,:]=C.correlate(img[i,j,:,:],img[i,j,:,:], output=None, mode='constant', cval=0.0, origin=0)
            sum1= cor1[i,0,:,:]+cor1[i,1,:,:]+cor1[i,2,:,:]
...
# =============================================================================
# def cor(img):
#     cor1=np.zeros((img.shape))
#     print(img.shape, "img shape")
#     print(C.correlate(img[0,:,:],img[0,:,:], output=None, mode='constant', cval=0.0, origin=0).shape)
...
#     #cor1[:,:,3]=C.correlate(img[:,:,0],img[:,:,1], output=None, mode='constant', cval=0.0, origin=0)
#     #cor1[:,:,4]=C.correlate(img[:,:,0],img[:,:,2], output=None, mode='constant', cval=0.0, origin=0)
#     #cor1[:,:,5]=C.correlate(img[:,:,1],img[:,:,2], output=None, mode='constant', cval=0.0, origin=0)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 25, 3, 1)#,dilation=2)
...
br=test(args, model, device, redtest_loader) #RED IS HORIZONTAL
