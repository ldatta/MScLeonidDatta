...
for seed_no in range(10):
    ...
    model2 = Netconv().to(device)
    model2.load_state_dict(torch.load('GLmodels/BaseGLseed{seed}.pt'.format(seed=seed_no+1)))
    ...
    model2.conv1.weight[:,0,:,:]=model2.conv1.weight[:,0,:,:]+model2.conv1.weight[:,1,:,:]
    model2.conv1.weight[:,1,:,:]=model2.conv1.weight[:,0,:,:]-model2.conv1.weight[:,1,:,:]
    model2.conv1.weight[:,0,:,:]=model2.conv1.weight[:,0,:,:]-model2.conv1.weight[:,1,:,:]
    
    model.to(device)
    accconvmin=test(args, model, device, redtest_loadertune)
    minacconv2=accconvmin
    ...
    for itr in range(5):
            ...
            for j2 in range(in_c2):
                ...
                if(i2>=j2):
                    if(i2!=j2):
                        ...
                        model.conv2.weight[:,i2,:,:]=model.conv2.weight[:,i2,:,:]-model.conv2.weight[:,j2,:,:]
                    print("Iter no",itr,"seed no",seed_no+1,"Checking Conv 2 channel no",i2,j2)
                    model.to(device)
        ...
        if(exchangefoundconv2):
            ...
            if(conv2_i!=conv2_j):
                model2.conv2.weight[:,conv2_i,:,:]=model2.conv2.weight[:,conv2_i,:,:]+model2.conv2.weight[:,conv2_j,:,:]
                model2.conv2.weight[:,conv2_j,:,:]=model2.conv2.weight[:,conv2_i,:,:]-model2.conv2.weight[:,conv2_j,:,:]
                model2.conv2.weight[:,conv2_i,:,:]=model2.conv2.weight[:,conv2_i,:,:]-model2.conv2.weight[:,conv2_j,:,:]
            ...
            for j3 in range(in_c3):
                ...
                if(i3>=j3):
                    if(i3!=j3):
                        ...
                        model.conv3.weight[:,i3,:,:]=model.conv3.weight[:,i3,:,:]-model.conv3.weight[:,j3,:,:]
                    print("Iter no",itr,"seed no",seed_no+1,"Checking Conv 3 channel no",i3,j3)
                    model.to(device)
        ...
        if(exchangefoundconv3):
            ...
            if(conv3_i!=conv3_j):
                model2.conv3.weight[:,conv3_i,:,:]=model2.conv3.weight[:,conv3_i,:,:]+model2.conv3.weight[:,conv3_j,:,:]
                model2.conv3.weight[:,conv3_j,:,:]=model2.conv3.weight[:,conv3_i,:,:]-model2.conv3.weight[:,conv3_j,:,:]
                model2.conv3.weight[:,conv3_i,:,:]=model2.conv3.weight[:,conv3_i,:,:]-model2.conv3.weight[:,conv3_j,:,:]
        model=model2
            ...
            for j4 in range(in_c4):
                ...
                if(i4>=j4):
                    if(i4!=j4):
                        ...
                        model.conv4.weight[:,i4,:,:]=model.conv4.weight[:,i4,:,:]-model.conv4.weight[:,j4,:,:]
                    print("Iter no",itr,"seed no",seed_no+1,"Checking Conv 4 channel no",i4,j4)
                    model.to(device)
        ...
        if(exchangefoundconv4):
            ...
            if(conv4_i!=conv4_j):
                model2.conv4.weight[:,conv4_i,:,:]=model2.conv4.weight[:,conv4_i,:,:]+model2.conv4.weight[:,conv4_j,:,:]
                model2.conv4.weight[:,conv4_j,:,:]=model2.conv4.weight[:,conv4_i,:,:]-model2.conv4.weight[:,conv4_j,:,:]
                model2.conv4.weight[:,conv4_i,:,:]=model2.conv4.weight[:,conv4_i,:,:]-model2.conv4.weight[:,conv4_j,:,:]
        model=model2   
            ...
            for j5 in range(in_c5):
                ...
                if(i5>=j5):
                    if(i5!=j5):
                        ...
                        model.conv5.weight[:,i5,:,:]=model.conv5.weight[:,i5,:,:]-model.conv5.weight[:,j5,:,:]
                    print("Iter no",itr,"seed no",seed_no+1,"Checking Conv 5 channel no",i5,j5)
                    model.to(device)
                    ...
                    if(accconv5>minacconv5):
                        minacconv5=accconv5
                        exchangefoundconv5=True
                        exchangefoundconv5_i=i5
                        exchangefoundconv5_j=j5 
                    if(i5!=j5):
                        model.conv5.weight[:,i5,:,:]=model.conv5.weight[:,i5,:,:]+model.conv5.weight[:,j5,:,:]
                        model.conv5.weight[:,j5,:,:]=model.conv5.weight[:,i5,:,:]-model.conv5.weight[:,j5,:,:]
                        model.conv5.weight[:,i5,:,:]=model.conv5.weight[:,i5,:,:]-model.conv5.weight[:,j5,:,:]
        
        if(exchangefoundconv5):
            conv5_i=exchangefoundconv5_i
            conv5_j=exchangefoundconv5_j
            if(conv5_i!=conv5_j):
                model2.conv5.weight[:,conv5_i,:,:]=model2.conv5.weight[:,conv5_i,:,:]+model2.conv5.weight[:,conv5_j,:,:]
                model2.conv5.weight[:,conv5_j,:,:]=model2.conv5.weight[:,conv5_i,:,:]-model2.conv5.weight[:,conv5_j,:,:]
                model2.conv5.weight[:,conv5_i,:,:]=model2.conv5.weight[:,conv5_i,:,:]-model2.conv5.weight[:,conv5_j,:,:]
        model=model2   
    
    np.save('GLtrainedKeepBestOfLayerGLseed{seed}Itr5tune_size2000tunelist'.format(seed=seed_no+1),listtune)
    np.save('GLtrainedKeepBestOfLayerGLseed{seed}Itr5tune_size2000testlist'.format(seed=seed_no+1),listtest)
    

print("First 100 of listtune",listtune[:100])
print("First 100 of listtest",listtest[:100])
#print(finallist)
print("GL trained! Tune Size 2000, Keep the best of layers , Iteration 5")
