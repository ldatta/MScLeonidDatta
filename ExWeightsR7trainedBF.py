...
for seed_no in range(10):
            ...
            for j5 in range(in_c5):
                ...
                if(i5>=j5):
                    if(i5!=j5):
                        ...
                        model.conv5.weight[:,i5,:,:]=model.conv5.weight[:,i5,:,:]-model.conv5.weight[:,j5,:,:]
                    print("Iter no",itr+1,"seed no",seed_no+1,"Checking Conv 5 channel no",i5,j5,"max till now",maxtillnow)
                    model.cuda()
    ...
    np.save('R7trainedalwaysKeepBetterGLseed{seed}Itr5tune_size100tunelist'.format(seed=seed_no+1),listtune)
    np.save('R7trainedalwaysKeepBetterGLseed{seed}Itr5tune_size100testlist'.format(seed=seed_no+1),listtest)
...
print(finallist)
print("R7 trained! Tune Size 100, KeepBetter, Iteration 5")
