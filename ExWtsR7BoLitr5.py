...
for seed_no in range(10):
    ...
    model2.load_state_dict(torch.load('R7models/BaseR7seed{seed}.pt'.format(seed=seed_no+1)))
    ...
    model.to(device)
    accconvmin=test(args, model, device, grntest_loadertune)
    ...
    np.save('R7trainedKeepBestOfLayerGLseed{seed}Itr5tune_size2000tunelist'.format(seed=seed_no+1),listtune)
    np.save('R7trainedKeepBestOfLayerGLseed{seed}Itr5tune_size2000testlist'.format(seed=seed_no+1),listtest)
    

print("First 100 of listtune",listtune[:100])
print("First 100 of listtest",listtest[:100])
#print(finallist)
print("R7 trained! Tune Size 2000, Keep the best of layers , Iteration 5")
